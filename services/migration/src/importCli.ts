 ...
 * with no tests, through two review rounds and a fix round.
 *
...
export interface ImportCliDependencies {
  /** The production factory in `cli.ts`; a fake in a test. */
...
export async function runImportCli(
  ...
  try {
    ...
    const importSummary = await runImport(context, parsedValues.tenant, {
      mappedRows,
    ...
    dependencies.logError(dryRun ? DRY_RUN_ABORT_MESSAGE : PARTIAL_WRITE_ABORT_MESSAGE);
    dependencies.logError(describeError(error));
    return { exitCode: 1, abortReason: error };
  }
}
